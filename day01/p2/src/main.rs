...
fn main() -> io::Result<()> {
    ...
    while let Some(Ok(line)) = lines.next() {
        ...
        if let Ok((first_digit, last_digit)) = find_digits(&line) {
           ...
           total += value;
           println!("{:?}: first digit = {:?}, last digit = {:?} ==> number = {:?} : total = {:?}", line, first_digit, last_digit, value, total); 
        } else {
...
#[cfg(test)]
mod tests {
    use super::{is_digit, find_digits};
